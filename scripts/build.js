...
if (process.env.NODE_ENV !== 'pre-dev') process.env.NODE_ENV = 'production';
...
// Set CSS and JS sources in all HTML files
function buildHtmlSources(stats, previousSizeMap) {
  walk(paths.appBuild, (error, files) => {
    if (error) {
      printErrors('Failed to compile.', [error]);
      process.exit(1);
    }

    var htmlFiles = files.filter(file => /\.(html)$/.test(file));
    var assets = stats.toJson().assets
      .filter(asset => /\.(js|css)$/.test(asset.name))
      .map(asset => {
        var assetName = process.env.NODE_ENV === 'pre-dev' ? "" : '/' + asset.name;
    ...
    htmlFiles.forEach(file => {
      ...
      assets.forEach(asset => {
        fileContents = fileContents.replace(asset.key, asset.name);
      });
...
function build(previousSizeMap) {
  if (process.env.NODE_ENV === 'pre-dev') {
    console.log('Creating a pre development build...');
  } else {
